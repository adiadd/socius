...
export function getAllResults(): Record<
	string,
	...
	for (const categoryId of categories) {
		...
		for (const scenarioId of scenarios) {
			const scenarioResult = {
				scenarioId,
				categoryId,
				modelResults: {} as Record<string, Record<string, ModelResult>>,
			};
			...
			const models = fs
				.readdirSync(scenarioPath, { withFileTypes: true })
				.filter((dirent) => dirent.isDirectory())
				.map((dirent) => dirent.name);
...
export function getSpecificResult(
	categoryId: string,
	scenarioId: string,
	modelId: string,
	date: string,
...
export async function getAllScenarioResults(): Promise<{
	[categoryId: string]: { [scenarioId: string]: ScenarioResult };
}> {
	return getAllResults();
}

// Server-side function to get latest results for a scenario
export async function getLatestResultsForScenario(
	categoryId: string,
	scenarioId: string,
): Promise<{ [modelId: string]: ModelResult }> {
	...
	for (const modelId in scenarioResults.modelResults) {
		...
		if (modelDates.length > 0) {
			...
			latestResults[modelId] =
				scenarioResults.modelResults[modelId][modelDates[0]];
		}
	}
