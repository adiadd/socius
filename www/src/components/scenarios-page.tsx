...
export default async function ScenariosPage() {
	...
	const scenarioCategories = Object.entries(allResults).map(
	...
	if (scenarioCategories.length === 0) {
		return (
			<div className="p-6 text-center">
				...
				<p>
					Make sure you have run some scenarios and have results in the
	...
	return (
		<>
			...
			<div className="border-b border-black dark:border-white p-6 text-center">
				<p className="text-secondary font-medium font-bold text-lg">
					scenarios
				</p>
				<p className="mt-2 max-w-2xl mx-auto">
					Explore scenarios designed to evaluate AI systems on their
					understanding of human values and ethical reasoning.
				</p>
			</div>
			...
			{scenarioCategories.map((category) => (
				<div
					key={category.id}
					className="border-b border-black dark:border-white p-6"
				>
			...
			<div className="p-6">
				<div className="flex flex-col md:flex-row">
					...
					<div className="w-full md:w-3/4">
						<p>
							we're continuously adding new scenarios - if you'd like to
							contribute, please create an issue and pull request on github ◡̈
						</p>
					</div>
				</div>
			</div>
...
function getScenarioDescription(
	...
	const descriptions: Record<string, Record<string, string>> = {
		ethics: {
			"trolley-problem":
				"evaluating how ai approaches the classic ethical dilemma",
		},
		society: {
			"universal-basic-income":
				"examining ai's perspective on economic policy alternatives",
		},
		// Add more scenarios as needed
	};

	return (
		descriptions[categoryId]?.[scenarioId] ||
		`${formatScenarioName(scenarioId)} scenario`
	);
}
